...
class SiecNeuronowa:
    def __init__(self, warstwa1, warstwa2):
    ...
    def uczenie(self, wejscia_treningowe, wyjscia_treningowe, iteracje_treningowe, tempo_uczenia):
        for t in range(iteracje_treningowe):
            ...
            wyjscia_warstwa1, wyjscia_warstwa2 = self.licz(wejscia_treningowe)

            # Liczymy błąd dla warstwy 2 (różnica między oczekiwanymi a przewidzianymi wyjściami)
            bledy_warstwa2 = wyjscia_treningowe - wyjscia_warstwa2
    ...
    def licz(self, wejscia):
        ...
        return wyjscia_warstwa1, wyjscia_warstwa2

    # Wypisanie wag sieci neuronowej
    def zwroc_wagi(self):
        return self.warstwa1.wagi, self.warstwa2.wagi
...
def testowanie(neuron, tempo_uczenia, iteracje, powtorzenia):
    wejscia_treningowe, wyjscia_treningowe, wejscia_testowe, wyjscia_testowe = wczytanie_danych()

    wagi_warstwa1 = 2 * np.random.random((13, neuron)) - 1
    wagi_warstwa2 = 2 * np.random.random((neuron, 1)) - 1
    layer1 = WarstwaNeuronow(wagi_warstwa1)
    layer2 = WarstwaNeuronow(wagi_warstwa2)
    neural_network = SiecNeuronowa(layer1, layer2)

    neural_network.uczenie(wejscia_treningowe, wyjscia_treningowe, iteracje, tempo_uczenia)

    for it in range(powtorzenia):
        wagi2_warstwa1, wagi2_warstwa2 = neural_network.zwroc_wagi()
        wejscia_treningowe, wyjscia_treningowe, wejscia_testowe, wyjscia_testowe = wczytanie_danych()
        warstwa1 = WarstwaNeuronow(wagi2_warstwa1)
        warstwa2 = WarstwaNeuronow(wagi2_warstwa2)
        neural_network = SiecNeuronowa(warstwa1, warstwa2)
        neural_network.uczenie(wejscia_treningowe, wyjscia_treningowe, iteracje, tempo_uczenia)

    ukryte, przewidywane_wyjscia = neural_network.licz(wejscia_testowe)
...
for i in range(1, 20):
    print(i)
    wektor_iteracji.append(i)
    suma = 0
    for k in range(5):
        skutecznosc = testowanie(neurony, lr, l_iteracje, i)
        suma += skutecznosc
    wektor_skutecznosci.append(suma / 5)
    if suma / 5 > maks:
        maks = suma / 5
        powtorzenie = i

plt.plot(wektor_iteracji, wektor_skutecznosci)
plt.title("Liczba uczeń a średnia skuteczność")
plt.xlabel("Ile razy powtarzano uczenie")
plt.ylabel("Średnia skuteczność")
plt.show()

print("Skuteczność: ", round(maks, 3) * 100, "%")
print("Powtorzenia uczeń: ", powtorzenie)
print("Neurony: ", neurony)
print("Tempo uczenia: ", lr)
print("Iteracje: ", l_iteracje)"""

suma = 0
print("Skuteczność sieci 2-warstwowej: ")
for k in range(10):
    skutecznosc = testowanie(2, 0.15, 24000, 17)
    print(round(skutecznosc, 3) * 100, "%")
    suma += skutecznosc

print("Średnia skuteczność sieci 2-warstwowej: ", round(suma/10, 3) * 100, "%")
